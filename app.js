...
function applyCustomVars(c) {
  ...
  // Sombras más marcadas sobre fondos oscuros (si no, son casi invisibles)
  s.setProperty('--shadow', dark ? '0 1px 2px rgba(0,0,0,.3), 0 10px 30px rgba(0,0,0,.4)' : '0 1px 2px rgba(38,36,32,.04), 0 10px 30px rgba(38,36,32,.07)');
}
function clearCustomVars() {
  const s = document.body.style;
  ['--bg','--panel','--text','--dim','--accent','--line','--line2','--accent-soft','--accent-tint','--shadow'].forEach(k => s.removeProperty(k));
  s.colorScheme = '';
}
...
function customThemeEditor() {
  ...
  const preview = `<div class="sum"><span class="red">● 1 vencido</span><span><b>${money0(816, pref('currency','USD'))}</b> / mes</span></div>
...
function upsertLoop(data) {
  const isNew = !data.id;
  if (isNew && !canAddLoop()) { showUpgradeModal('limit'); return; }
...
function duplicateLoop(id) {
  if (!canAddLoop()) { showUpgradeModal('limit'); return; }
  const loop = loops.find(l => l.id === id); if (!loop) return;
...
function renderAjustes(v) {
  ...
  v.innerHTML = `
    ...
    <div class="brk"><h4>Datos y respaldo</h4>
      ...
      <button class="opt" id="o-photo">${svg('camera')} Importar por foto ✨ <span class="opt-tag">Full</span></button>
  ...
  v.querySelector('#o-ics').onclick = () => { if (!loops.length) { alert('No hay Loops que exportar.'); return; } downloadICS(loops, 'loopkeeper'); };
  ...
  v.querySelector('#o-photo').onclick = importByPhoto;
  ...
  if (upgradeBtn) upgradeBtn.onclick = () => showUpgradeModal();
}
...
function openForm(loop, opts) {
  ...
  modal.innerHTML = `
    ...
    <div class="field"><div class="toggle-row">
      <div><div class="tl">⭐ Importante</div><div class="ts">Sube al inicio de su lista y aparece en la pestaña “Importantes”.</div></div>
...
(() => {
  ...
  // Redimensiona a 160x160 (recorte cuadrado centrado) para no llenar el almacenamiento
  const draw = (src, w, h) => {
  ...
  inp.onchange = async () => {
    ...
    inp.value = '';
    try {
      // createImageBitmap respeta la orientación EXIF (fotos giradas en iOS)
      const bmp = await createImageBitmap(file, { imageOrientation: 'from-image' });
      draw(bmp, bmp.width, bmp.height);
      if (bmp.close) bmp.close();
    } catch (e) {
      const reader = new FileReader();
      reader.onload = () => { const img = new Image(); img.onload = () => draw(img, img.width, img.height); img.src = reader.result; };
...
document.addEventListener('keydown', (e) => {
  ...
  if (ov && !ov.hidden) { ov.hidden = true; return; }   // cierra el modal si está abierto
  closeDrawer();
});
...
function buildBackup() {
  return {
    ...
    prefs: { defaultNotify: pref('defaultNotify','3'), defaultAuto: pref('defaultAuto','0'), theme: pref('theme','marfil'), layout: pref('layout','list'), customTheme: getCustomTheme(), avatar: localStorage.getItem(AVATAR_KEY) || undefined },
...
function applyBackup(data) {
  ...
  if (prefs) {
    ...
    if (prefs.avatar) { localStorage.setItem(AVATAR_KEY, prefs.avatar); applyAvatar(); }
...
/* ---------- Importar por foto (IA — solo Full) ---------- */
const photoFile = document.getElementById('photo-file');
function importByPhoto() {
  if (!getSession()) { showAuthScreen(); return; }
  if (_plan !== 'full') { showUpgradeModal('photo'); return; }
...
function showPhotoLoading() {
  ...
  m.innerHTML = `<div style="text-align:center;padding:20px 8px">
    <div class="spinner"></div>
    <h2 style="margin-top:18px">Leyendo tu foto…</h2>
    <p class="modal-sub">La IA está identificando tus loops. Tarda unos segundos.</p></div>`;
  ov.hidden = false;
}
if (photoFile) photoFile.onchange = async (e) => {
  const file = e.target.files[0]; photoFile.value = ''; if (!file) return;
  let dataUrl;
  try { dataUrl = await downscaleImage(file, 1500, 0.8); }
  catch (err) { alert('No pude leer esa imagen. Prueba con otra foto.'); return; }
  showPhotoLoading();
  try {
    const r = await fetch(`${SB.url}/functions/v1/import-photo`, { method: 'POST', headers: sbHeaders(), body: JSON.stringify({ image: dataUrl }) });
    const j = await r.json().catch(() => ({}));
    if (r.status === 403 || j.error === 'full_only') { closeModal(); showUpgradeModal('photo'); return; }
    if (r.status === 401) { closeModal(); showAuthScreen(); return; }
    if (r.status === 503 || j.error === 'no_key') { closeModal(); alert('La lectura por foto se está activando. Vuelve a intentarlo muy pronto. 🚀'); return; }
    if (!r.ok || !Array.isArray(j.loops)) { closeModal(); alert(j.error || 'No pude leer la foto. Prueba con una más clara y de frente.'); return; }
    if (!j.loops.length) { closeModal(); alert('No encontré loops en la foto. Funciona mejor con una lista, factura o recibo claros.'); return; }
    showPhotoReview(j.loops);
  } catch (err) { closeModal(); alert('Hubo un problema de conexión. Inténtalo de nuevo.'); }
};
function normalizeImported(it) {
  const l = normalizeLoop({
    title: it.title,
    amount: it.amount,
    currency: CURRENCIES[it.currency] ? it.currency : pref('currency', 'USD'),
    category: it.category,
    icon: ICON[it.icon] ? it.icon : (CATEGORIES[it.category] ? CATEGORIES[it.category].icon : 'repeat'),
    recurrence: it.recurrence,
    nextDate: it.nextDate || offsetDate(7),
    notes: it.notes,
    notifyDaysBefore: parseInt(pref('defaultNotify', '3'), 10) || 3,
    autoPay: false,
  });
  l._sel = true;
  return l;
...
function impMeta(l) {
  const parts = [];
  if (l.amount != null) parts.push(money(l.amount, l.currency));
  parts.push(RECURRENCE[l.recurrence] ? RECURRENCE[l.recurrence].label : 'Sin repetir');
  parts.push(catOf(l).label);
  return parts.join(' · ');
}
function showPhotoReview(items) {
  _photoLoops = items.map(normalizeImported);
  renderPhotoReview();
}
function renderPhotoReview() {
  const ov = document.getElementById('modal-overlay'), m = document.getElementById('modal');
  const rows = _photoLoops.map((l, i) => `
    <label class="imp-row">
      <input type="checkbox" data-i="${i}" ${l._sel ? 'checked' : ''}>
      <span class="imp-ic">${svg(l.icon)}</span>
      <span class="imp-body">
        <span class="imp-title">${escapeHtml(l.title)}</span>
        <span class="imp-meta">${escapeHtml(impMeta(l))}</span>
      </span>
    </label>`).join('');
  const n = _photoLoops.filter(l => l._sel).length;
  m.innerHTML = `
    <h2>Encontré ${_photoLoops.length} loop${_photoLoops.length === 1 ? '' : 's'}</h2>
    <p class="modal-sub">Revisa y desmarca los que no quieras. Podrás editar cada uno después.</p>
    <div class="imp-list">${rows}</div>
    <div class="modal-actions" style="margin-top:14px">
      <button class="btn btn-ghost" id="imp-cancel">Cancelar</button>
      <button class="btn btn-primary" id="imp-add"${n ? '' : ' disabled'}>Añadir ${n} loop${n === 1 ? '' : 's'}</button>
    </div>`;
  ov.hidden = false;
  m.querySelectorAll('.imp-row input').forEach(c => c.onchange = () => { _photoLoops[+c.dataset.i]._sel = c.checked; renderPhotoReview(); });
  m.querySelector('#imp-cancel').onclick = closeModal;
  m.querySelector('#imp-add').onclick = () => {
    const sel = _photoLoops.filter(l => l._sel);
    if (!sel.length) { closeModal(); return; }
    sel.forEach(l => {
      const { _sel, ...rest } = l;
      const data = { ...rest, id: uid(), history: [] };
      loops.push(data);
      if (getSession()) serverSaveLoop(data);
    });
    save(); render(); closeModal();
    alert(`${sel.length} loop${sel.length === 1 ? '' : 's'} añadido${sel.length === 1 ? '' : 's'} ✓`);
  };
}
...
function exportData() {
  ...
  a.download = `loopkeeper-backup-${fmtDate(todayMid())}.json`; a.click();
...
function downloadICS(list, name) {
  ...
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
  a.download = (name || 'loopkeeper') + '.ics'; a.click();
  setTimeout(() => URL.revokeObjectURL(a.href), 2000);
}
...
async function signUp(email, password) {
  ...
  const j = await r.json().catch(() => ({}));
  if (!r.ok) throw new Error(j.error || 'Error al crear cuenta');
  // Confirmada → iniciar sesión directo
...
async function syncDown() {
  ...
  const before = JSON.stringify(loops);   // foto de lo local ANTES de la red
  try {
    ...
    // Si el usuario tocó algo mientras descargábamos (arranque sin esperar a la red),
    // no pisamos su cambio: subimos lo local y dejamos que se reconcilie.
    if (JSON.stringify(loops) !== before) { syncUp(); return; }
...
async function serverSaveLoop(loop) {
  ...
  try {
    ...
    if (!res.ok) { const body = await res.text(); if (body.includes('free_plan_limit')) showUpgradeModal('limit'); }
  } catch(e) {}
}
...
async function fetchShared() {
  ...
  try {
    const r = await fetch(`${SB.url}/rest/v1/loop_shares?grantee_id=eq.${s.user.id}&select=permission,loops(id,client_id,data,owner_id)`, { headers: sbHeaders() });
    if (!r.ok) { sharedLoops = []; return; }
    const rows = await r.json();
    sharedLoops = rows.filter(x => x.loops && x.loops.data).map(x => ({ ...x.loops.data, id: x.loops.client_id, _shared: true, _perm: x.permission, _owner: x.loops.owner_id, _uuid: x.loops.id }));
  } catch(e) { sharedLoops = []; }
}
...
async function renderCompartidos(v) {
  ...
  el.querySelectorAll('.chk[data-cid]').forEach(b => b.onclick = (e) => { e.stopPropagation(); markDoneShared(b.dataset.cid); });
}
async function markDoneShared(cid) {
  const loop = sharedLoops.find(l => l.id === cid); if (!loop || !SB.url) return;
  const s = await ensureSession(); if (!s) return;
  const next = nextCycle(loop);
  const upd = { ...loop }; const uuid = upd._uuid;
  delete upd._shared; delete upd._perm; delete upd._owner; delete upd._uuid;
  upd.history = (upd.history || []).concat([upd.nextDate]);
  if (next) upd.nextDate = next;
  try {
    await fetch(`${SB.url}/rest/v1/loops?id=eq.${uuid}`, {
      method: 'PATCH', headers: { ...sbHeaders(), Prefer: 'return=minimal' },
      body: JSON.stringify({ next_date: upd.nextDate, data: upd })
    });
  } catch(e) {}
  const v = document.getElementById('view'); if (v && activeSection === 'compartidos') renderCompartidos(v);
}
...
function showUpgradeModal(reason) {
  ...
  m.innerHTML = `
    <h2>Hazte Full</h2>
    <p class="modal-sub">${reason === 'limit' ? 'Llegaste a los 10 loops del plan gratis. ' : reason === 'photo' ? 'Importar por foto es una función Full. ' : ''}Loops ilimitados, importar por foto, compartir sin límites y más.</p>
  ...
  ov.hidden = false;
  m.querySelectorAll('.plan-card').forEach(c => c.onclick = () => { ov.hidden = true; startCheckout(c.dataset.interval); });
  m.querySelector('#btn-cancel-full').onclick = () => { ov.hidden = true; };
  ov.onclick = (e) => { if (e.target === ov) ov.hidden = true; };
}
async function startCheckout(interval) {
  const s = await ensureSession(); if (!s) { showAuthScreen(); return; }
  try {
    const r = await fetchTimeout(`${SB.url}/functions/v1/create-checkout`, {
      method: 'POST', headers: sbHeaders(), body: JSON.stringify({ interval })
    });
    const j = await r.json().catch(() => ({}));
    if (r.ok && j.url) { location.href = j.url; return; }
    alert(j.error || 'Los pagos se activan muy pronto. ¡Gracias por tu interés en Full! 🚀');
  } catch(e) {
    alert('Los pagos se activan muy pronto. ¡Gracias por tu interés en Full! 🚀');
  }
}
...
function checkDue() {
  ...
  let prev = {}; try { prev = JSON.parse(localStorage.getItem(NOTIFY_SENT_KEY)) || {}; } catch (e) {}
  const today = fmtDate(todayMid());
  // Solo conservamos las claves de hoy: evita que el registro crezca sin fin.
  const sent = {};
  Object.keys(prev).forEach(k => { if (k.endsWith('@' + today)) sent[k] = true; });
  loops.forEach(l => {
    const st = statusOf(l);
...
function renderAuthScreen(tab) {
  ...
  scr.querySelector('#auth-submit').onclick = async () => {
    ...
    if (tab === 'signup' && pwd.length < 6) { showErr('La contraseña debe tener al menos 6 caracteres.'); return; }
