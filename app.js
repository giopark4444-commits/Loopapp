...
function cycleDays(loop) {
  return CYCLE_DAYS[loop.recurrence] || Math.max((loop.notifyDaysBefore || 3) * 2, 14);
}
...
function nextCycle(dateStr, recurrence) {
  ...
  do {
    if (STEP_DAYS[recurrence]) d.setDate(d.getDate() + STEP_DAYS[recurrence]);
    else if (STEP_MONTHS[recurrence]) d.setMonth(d.getMonth() + STEP_MONTHS[recurrence]);
    else return null;
  } while (d <= today);
  return fmtDate(d);
}
function occursOn(loop, date) {
  const a = parseDate(loop.nextDate), r = loop.recurrence;
  if (r === 'none') return fmtDate(date) === loop.nextDate;
  if (STEP_DAYS[r]) { const diff = Math.round((date - a) / 86400000); return diff % STEP_DAYS[r] === 0; }
  const per = STEP_MONTHS[r];
  if (!per) return false;
...
function openForm(loop) {
  ...
  if (isEdit) modal.querySelector('#f-ics').onclick = () => downloadICS([data], 'loop');
...
function loopVEvent(l) {
  const RR = { weekly:'FREQ=WEEKLY', biweekly:'FREQ=WEEKLY;INTERVAL=2', monthly:'FREQ=MONTHLY',
    bimonthly:'FREQ=MONTHLY;INTERVAL=2', quarterly:'FREQ=MONTHLY;INTERVAL=3', biannual:'FREQ=MONTHLY;INTERVAL=6', yearly:'FREQ=YEARLY' }[l.recurrence];
  const sum = l.title + (l.amount ? ' — ' + money(l.amount) : '');
  const cat = CATEGORIES[l.category] ? CATEGORIES[l.category].label : '';
  const desc = [cat, RECURRENCE[l.recurrence].label, payType(l) === 'auto' ? 'pago automático' : ''].filter(Boolean).join(' · ');
